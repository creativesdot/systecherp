...
export default class WorkOrder extends Component {
  ...
  searchRandomUser = async () => {
    ...
    db.transaction(tx => {
      tx.executeSql('SELECT * FROM table_user', [], (tx, results) => {
        ...
        fetch("http://106.51.49.166:8090/MMS/rest/api/latest/android/workorderregister", {
          method: 'POST',
          headers: {
        ...
        <View style={styles.container}>
          ...
          <FlatGrid
            ...
            renderItem={({ item, index }) => (
              <TouchableOpacity activeOpacity={.7} onPress={() => this.props.navigation.navigate('WorkOrderEdit', {
                details: item, WorkOrder: this, user_id: FlatListItems[0].user_id, visibility: this.state.visibility, reject: this.state.reject, approved: this.state.approved
              })}>
                <Card>
                  ...
                  <CardItem footer>
                    ...
                    <Button rounded small primary>
                      <Text style={styles.ButText} onPress={() => { this.props.navigation.navigate('ItemDetails', { id: item.transno, type: "Work Order" }) }}>Item Details</Text>
                    </Button>
                  </CardItem>
                </Card>
              </TouchableOpacity>
  ...
  approveClickListener = (transaction_id, status_approve) => {
    ...
    //alert(params);
    fetch("http://106.51.49.166:8090/MMS/rest/api/latest/android/workorder_approval", {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
