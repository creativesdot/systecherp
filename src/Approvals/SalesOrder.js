...
export default class SalesOrder extends Component {
        ...
        <View style={styles.container}>
          ...
          <FlatGrid
            ...
            renderItem={({ item, index }) => (
              <TouchableOpacity activeOpacity={.7} onPress={() => this.props.navigation.navigate('SalesOrderEdit', {
                details: item, SalesOrder: this, user_id: FlatListItems[0].user_id, visibility: this.state.visibility, reject: this.state.reject, approved: this.state.approved
              })}>
                <Card>
                  ...
                  <CardItem footer>
                    ...
                    <Button rounded small primary>
                      <Text style={styles.ButText} onPress={() => { this.props.navigation.navigate('ItemDetails', { id: item.transno, type: "Product Order" }) }}>Item Details</Text>
                    </Button>
                  </CardItem>
                </Card>
              </TouchableOpacity>
  ...
  approveClickListener = (transaction_id, status_approve) => {
    ...
    //alert(params);
    fetch("http://106.51.49.166:8090/MMS/rest/api/latest/android/productorder_approval", {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
