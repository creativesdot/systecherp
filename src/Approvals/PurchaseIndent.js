...
export default class PurchaseIndent extends Component {
  ...
  ListEmptyView = () => {
    return (
      <View style={styles.MainContainer}>
        <Text style={{ textAlign: 'center', fontSize: 20, justifyContent: 'center' }}> Data Not Available</Text>
      </View>
        ...
        <View style={styles.container}>
          ...
          <FlatGrid
            ...
            renderItem={({ item, index }) => (
              <TouchableOpacity activeOpacity={.7} onPress={() => this.props.navigation.navigate('PurchaseIndentEdit', {
                details: item, PurchaseIndent: this, user_id: FlatListItems[0].user_id, visibility: this.state.visibility, reject: this.state.reject, approved: this.state.approved
              })}>
                <Card>
                  ...
                  <CardItem>
                    <Body>
                      ...
                      <View style={styles.Supplier}>
                        ...
                        <Text style={styles.SupplierData}>{item.purchasetype}</Text>
                      </View>
                    </Body>
                  </CardItem>
                  <CardItem footer>
  ...
  approveClickListener = (transaction_id, status_approve) => {
    ...
    //alert(params);
    fetch("http://106.51.49.166:8090/MMS/rest/api/latest/android/purchaseindent_approval", {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
